...
const rowSchema = z.object({
  ...
  neighborhood: z.string().refine((val) => val.trim().length > 0, {
    message:
      "O campo neighborhood é obrigatório e deve ser uma string não vazia.",
  }),
  street: z.string().refine((val) => val.trim().length > 0, {
  ...
  streetNumber: z
    ...
    .refine((val) => val.trim().length > 0, {
      message:
        "O campo streetNumber é obrigatório e deve ser uma string não vazia.",
    })
    .transform((val) => Number(val))
    .refine(
      (val) => !isNaN(val),
      "O campo streetNumber deve ser um número válido.",
    ),
    ...
    for (const [index, row] of csvData.entries()) {
      try {
        ...
        if (error instanceof z.ZodError) {
          errors.push(
            `Erro na linha ${index + 1}: ${error.errors.map((e) => e.message).join(", ")}`,
          );
        }
      }
    }
